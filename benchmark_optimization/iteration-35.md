# Iteration 35 — Harness-suite: SSH host entry absent, stop condition fired
...
- `harness_architectures`: 0 cases, empty `by_architecture`, `by_test`,
  ...
  iterations (21-34) stopped at network-unreachable (exit 255) or missing
  SSH host entry; iteration 32 first identified the missing
  `sugarcube-benchmark` host entry; iteration 33 reported it restored but
  the benchmark still failed with hostname resolution failure; iteration 34
  confirmed the entry is again absent.
...
- `git pull --ff-only origin main`: already up to date (HEAD 0dba25d).
...
- SSH config contains a `sugarcube-benchmark` host entry: **not
  confirmed**. The config file defines hosts `servarr`, `homeassistant`,
  and `windows-pc` only. The `sugarcube-benchmark` host entry is absent.
  This is the same condition that stopped iterations 32 and 34.
...
pattern from iterations 21-34. Per the goal's stop conditions and the
...
been absent from the SSH config for fifteen consecutive iterations
(21-35, with iterations 32, 34, and 35 explicitly identifying the
missing entry and iteration 33 reporting a restoration that did not
persist). Operator action required to add the `sugarcube-benchmark` host
entry to `/opt/data/home/.ssh/config` before the next scheduled
benchmark attempt.
