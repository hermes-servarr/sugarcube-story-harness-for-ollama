...
corpus (managed process `proc_2bb6bccab0c8`, PID 6519). The SSH command
...
This matches the network-unreachable pattern from iterations 21-38.
...
for nineteen consecutive iterations (21-39). Iteration 38 and this
iteration confirmed the `sugarcube-benchmark` SSH host IS present and
resolvable via `ssh -G` (host, user, hostname, port 22 all resolved),
but the actual SSH connection still fails with DNS resolution failure
(exit 255). The `sugarcube-benchmark` hostname cannot be resolved at
the OS DNS level despite being present in the SSH config file. The
SSH config file has not been modified since Aug 28 11:32.
