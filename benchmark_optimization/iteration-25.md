# Iteration 25 — Harness-suite: validate frozen corpus and attempt architecture baseline
...
- `harness_architectures`: 0 cases, empty `by_architecture`, `by_test`,
  ...
  iterations (20-24) stopped at network-unreachable (exit 255).
...
No existing case needs further revision in this experiment. The
protected test `test_refactor_corpus_has_fixed_core_and_canary_sizes`
...
SSH command exited with code 255: the benchmark PC was unreachable
(connection timed out). This matches the network-unreachable pattern
from iterations 20-24. Per the goal's stop conditions, this is a
disconnect stop condition and must not be retried. Operator action
...
for five consecutive iterations (21-25). Operator action required to
